...
def predict_match(match, region, tagline: str):
    ...
    for participant in participants:
        ...
        try:
            ...
            tagline = participant["tagLine"]
        winrate_list = api_calls.get_winrates(summonerName, region, tagline)["winrate"]
        mastery_list = api_calls.get_masteries(summonerName, region, tagline)["mastery"]
        mastery = 0
...
def get_current_match_prediction(summonerName: str, region: str, tagline: str):
    ...
    match = dict()
    match = api_calls.get_live_match(summonerName, region, tagline)
    ...
    response = {}
    prediction = predict_match(match, region1, tagline)
...
def get_last_match_prediction(summonerName: str, region: str, tagline:str):
    # Get last match
    print("-----Getting last match-----")
    match = api_calls.get_past_matches(summonerName, region, 1,tagline)[0]
    prediction = predict_match(match, region,tagline)
